...
public class Partie {
	...
	public void deplacer(Joueur joueur,int positionDepart,int positionArrivee,int de){
		...
		ArrayList<Integer> listeBarre = this.plateau.indicSortirPionsBarre(joueur, de);
		ArrayList<Integer> listeDeplacement = this.plateau.indicationDeplacement(positionDepart, de);
		 ArrayList<Integer> listeManger = this.plateau.indicationManger(positionDepart,de);
		 int colonne = 0;
		...
		default : break;
		 ...
		 if(this.plateau.isInList(listeDeplacement, positionArrivee)==true){
			 this.plateau.deplacer(positionDepart, positionDepart-positionArrivee);
		 }
		 if(this.plateau.isInList(listeManger, positionArrivee)==true){
			 this.plateau.manger(positionDepart, positionDepart-positionArrivee);
		 }
		 }
		 else{
			 if(this.plateau.isInList(listeBarre, positionArrivee)==true){
