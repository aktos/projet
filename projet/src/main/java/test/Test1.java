...
/*	
	...
	public void testPartie(){
		...
		int de=4;
		//boolean listeBarre = grille.indicSortirPionsBarre(joueur, de);
		/*ArrayList<Integer> listeDeplacement = grille.indicationDeplacement(positionDepart, de);
		 ArrayList<Integer> listeManger = grille.indicationManger(positionDepart,de);
		 int colonne = 0;
		 if(grille.sizeColonne(colonne)==0){
				
			 if(grille.isInList(listeDeplacement, positionArrivee)==true){
				 grille.deplacer(positionDepart, positionDepart-positionArrivee);
			 }
			 if(grille.isInList(listeManger, positionArrivee)==true){
				 grille.manger(positionDepart, positionDepart-positionArrivee);
			 }
			 
			 System.out.println("Deplacement effectue !");
			 }*/
		
		/* if(grille.isInList(listeBarre, positionArrivee)==true){
			// grille.sortirPionsBarre(joueur, positionArrivee);
			 System.out.println("OK");
		 }
		 else {System.out.println("cette position n'est pas possible");}
		 	*/
		//System.out.println(grille.listToString(listeBarre," ","."," "));
		//System.out.println(grille.isInList(listeBarre, positionArrivee));
	jeu.lancerPartie();
